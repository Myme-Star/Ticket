...
public class CartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        /*for(int i=0;i<strVals.length;i++){
            int a=0;
            if(strVals[i]==null) strVals[i]=Integer.toString(a);
        }*/

        try {
            Class.forName("org.apache.derby.jdbc.ClientDriver");
            String driverURL = "jdbc:derby://localhost:1527/Kumiawase";
            ...
            for (int i = 0; i < strVals.length; i++) {
                ...
                int idFromCB = i+1;
                //if(strVals[i] != null ? Integer.toString(0) != null : strVals[i].equals(Integer.toString(0)))
                // その商品の情報（名前、価格）をDBから取り出す。合計金額も計算する。
                String sql = "select * from T_SHOHIN where SHOHIN_ID ="
                        + idFromCB;
                ResultSet rs = stmt.executeQuery(sql);
                if(strVals[i] == "") strVals[i] = Integer.toString(0);
               
                if (rs.next() && !"0".equals(strVals[i])) {
                    ...
                    int p = new Integer(rs.getInt("PRICE"));
                    int price = k*p;
                    record.put("price", price);
                    record.put("tanka",rs.getInt("PRICE"));
                    record.put("kosuu",strVals[i]);
                    total = total + price;
                    list.add(record);
                }
